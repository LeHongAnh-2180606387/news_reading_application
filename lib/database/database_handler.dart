...
class DatabaseHandler {
  ...
  void addBookmark(BuildContext context, DataModel model) {
    var random = Random();
    const allChars =
        'AaBbCcDdlMmNnOoPpQqRGgHhIiJj465juds9576fhaf453765ydfgajKkL234567890';
    final key =
        List.generate(50, (index) => allChars[random.nextInt(allChars.length)])
            .join();
    ...
    try {
      userData
          ...
          .child(key)
          .set(model.toMap())
          .then((value) => showMessage(context))
  ...
  void showMessage(BuildContext context) async {
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content:
            const Text("Added on bookmarks", style: TextStyle(fontSize: 16)),
        actions: <Widget>[
          TextButton(
            onPressed: () {
